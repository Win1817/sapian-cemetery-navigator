...
export const GraveList = ({ onEdit }: GraveListProps) => {
  ...
  const fetchGraves = async () => {
    ...
    if (error) {
      ...
      // Fetch lots with block information
      const { data: allLots, error: lotsError } = await (supabase.from("lots") as any).select("id, lot_number, block_id(block_name)");
      
      if (!lotsError && allLots && data) {
        // Create a map of lot IDs to display names
        const lotMap = new Map(allLots.map((lot: any) => {
          const blockName = lot.block_id?.block_name || "Unknown Block";
          return [lot.id, `${blockName} - Lot ${lot.lot_number}`];
        }));
        
        // Add lot display names to graves
        const gravesWithLots = data.map(grave => {
          const lotDisplayName = grave.lot_block_id ? lotMap.get(grave.lot_block_id) : undefined;
          return {
            ...grave,
            lot_display_name: lotDisplayName
          };
        });
        setGraves(gravesWithLots);
      } else {
        setGraves(data || []);
      }
    }
          ...
          <Card key={grave.id} className="p-3 sm:p-4 shadow-soft hover:shadow-medium transition-shadow overflow-hidden">
            ...
            <div className="text-xs sm:text-sm text-muted-foreground mb-2 space-y-1">
              ...
              <div className="flex items-center space-x-1">
                ...
                <p>{grave.lot_display_name ? `📍 ${grave.lot_display_name}` : "No Location Assigned"}</p>
              </div>
              {grave.date_of_death && (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-lg">{selectedGrave?.lot_display_name ? selectedGrave.lot_display_name : "No Location Assigned"}</p>
              </div>
            </div>
