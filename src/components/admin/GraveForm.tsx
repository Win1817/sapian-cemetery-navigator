...
export const GraveForm = ({ grave, onClose }: GraveFormProps) => {
    ...
    if (!isLot) {
      const fetchLots = async () => {
        const { data, error } = await (supabase.from("lots") as any)
          .select("id, lot_number, is_available, block_id(block_name)")
          .eq("is_available", true) // Only fetch available lots
          .order("lot_number");
  ...
  const handleLotSelect = (lotId: string) => {
    const lot = availableLots.find((l: any) => l.id === lotId);
    if (lot) {
      setSelectedLotId(lotId);
      const blockName = lot.block_id?.block_name || "Unknown Block";
      const lotDisplay = `${blockName} - Lot ${lot.lot_number}`;
      
      toast({
        title: "Location Set",
        description: `Assigned to ${lotDisplay}`,
      });
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {!isLot && (
        <div className="space-y-2">
           ...
           {grave?.lot_block_id ? (
             <div className="w-full p-3 rounded-md border border-green-300 bg-green-50">
               <p className="text-sm font-medium text-green-900">
                 ✓ Already assigned to a lot
               </p>
               <p className="text-xs text-green-700 mt-1">
                 To change the assignment, unassign from Lot Management first.
               </p>
             </div>
           ) : (
                ...
                {availableLots.length === 0 ? (
                  ...
                  availableLots.map((lot: any) => (
                    <SelectItem key={lot.id} value={lot.id}>
                      {lot.block_id?.block_name} - Lot {lot.lot_number}
                    </SelectItem>
                  ))
                )}
              </SelectContent>
          ...
          <p className="text-[0.8rem] text-muted-foreground">
            {grave?.lot_block_id ? "This grave is already assigned to a lot." : "Assigning to a lot automatically sets the location."}
          </p>
        </div>
      )}
